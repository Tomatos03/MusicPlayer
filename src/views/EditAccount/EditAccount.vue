<template>
    <div class="edit-account">
        <h1>编辑个人信息</h1>
        <div class="container">
            <el-form :model="accountFrom" class="account-from">
                ...
                <el-form-item label="生日：">
                    <el-select size="large"
                        v-for="(select, index) in accountFrom.birthday"
                            ...
                            style="width: 150px"
                            :key="index">
                        <el-option v-for="num in select.range"
                                    :key="num"
                                    :value="num"
                                    :label="num"/>
                    </el-select>
                ...
                <el-form-item label="地区：">
                    <el-select v-for="location in accountFrom.location"
                                v-model="location.value"
                                size="large">
                        <el-option v-for="info in location.data"
                                    :key="info.name"
                                    :value="0"
                                    :label="info.name"/>
                    </el-select>
...
<script setup>
    ...
    watch(() => accountFrom.value.birthday.year.value, (newYear, oldYear) => {
        accountFrom.value.birthday.day.range = getAbleSelectDays(newYear, accountFrom.value.birthday.month.value);
    });

    watch(() => accountFrom.value.birthday.month.value, (newMonth, oldMonth) => {
        accountFrom.value.birthday.day.range = getAbleSelectDays(accountFrom.value.birthday.year.value, newMonth);
    });
</script>
<style scoped>
    .el-select{
